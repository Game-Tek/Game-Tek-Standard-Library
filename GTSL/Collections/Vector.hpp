...
{	
	...
	public:
		 ...
		 */
		Vector(Vector&& other) noexcept requires std::move_constructible<ALLOCATOR>
		: allocator(GTSL::MoveRef(other.allocator)), length(other.length),
		...
		Result<uint32> Find(const T& obj) const noexcept
		{
			for (uint32 i = 0; i < GetLength(); ++i)
			{
				if (obj == data[i])
				{
					return Result<uint32>(MoveRef(i), true);
				}
		...
		friend bool operator<(const uint32 i, const Vector& vec) { return i < vec.GetLength(); }
...
#pragma warning(default : 4648)
		...
		static void buildCopyArray(const T* from, T* to, const length_type elementCount)
	...
	public:
		...
		MultiVector(const uint32 minElements, const ALLOCATOR& allocator = ALLOCATOR())
		: allocator(allocator)
		{
