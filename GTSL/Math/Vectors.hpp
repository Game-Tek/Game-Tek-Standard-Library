...
namespace GTSL
{
	...
	public:
		...
		constexpr Vector2(float a) : values{ a, a } {}
		constexpr Vector2(float x, float y) : values{ x, y } {}
		...
		[[nodiscard]] constexpr float X() const { return values[0]; }
		[[nodiscard]] constexpr float Y() const { return values[1]; }

		float* GetData() { return values; }
		[[nodiscard]] const float* GetData() const { return values; }

		float operator[](const uint8 i) const { return values[i]; }
		float& operator[](const uint8 i) { return values[i]; }
	...
	public:
		...
		constexpr explicit Vector3(const float a) : values{ a, a, a } {}

		constexpr Vector3(const Vector2 a, const float z) : values{ a.X(), a.Y(), z } {}
		constexpr Vector3(const float x, const float y, const float z) : values{ x, y, z } {}
		...
		friend Vector3 operator*(const float lhs, const Vector3& rhs) { return { rhs.X() * lhs, rhs.Y() * lhs, rhs.Z() * lhs }; }

		float& X() { return values[0]; }
		float& Y() { return values[1]; }
		float& Z() { return values[2]; }

		[[nodiscard]] float X() const { return values[0]; }
		[[nodiscard]] float Y() const { return values[1]; }
		[[nodiscard]] float Z() const { return values[2]; }
	...
	public:
		...
		Vector4(const float a) : values{ a, a, a, a } {}
		Vector4(const float x, const float y, const float z, const float w) : values{ x, y, z, w } {}

		Vector4(const Vector3 other, const float w) : Vector4(other.X(), other.Y(), other.Z(), w) {}
		...
		Vector4 operator/(float other) const { const auto div = 1.f / other; return { X() * div, Y() * div, Z() * div, W() * div }; }
		Vector4& operator/=(float other) { X() /= other; Y() /= other; Z() /= other; W() /= other; return *this; }
		...
		[[nodiscard]] float Y() const { return values[1]; }
		[[nodiscard]] float Z() const { return values[2]; }
		[[nodiscard]] float W() const { return values[3]; }
