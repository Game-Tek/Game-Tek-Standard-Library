...
#include "../Core.hpp"
#include "../DataSizes.hpp"
#include "../Result.hpp"
#include "../Unicode.hpp"
#include "../Math/Math.hpp"
#include "GTSL/Collections/Range.hpp"
...
namespace GTSL
{
	...
	* \return The string length including the null terminator.
	...
	//typedef Range<const char*> StringView;
	typedef Range<const char8_t*> StringView;
	...
	template<class S>
	void ToString(S& string, float num) {
		uint8 i = 30, len = 0;
		char8_t str[32]; str[31] = u8'\0'; bool isNegative = false;
		...
		float fpart = num - static_cast<float>(ipart);
	...
	template<>
	inline Result<float> ToNumber(Range<const char8_t*> numberString) {
		float value = 0; uint64 c = numberString.GetBytes() - 1/*because of inverse parse;

		{
			...
			while (c <= numberString.GetBytes() - 1) {
				...
				default: return Result<float>(0, false);
				}
