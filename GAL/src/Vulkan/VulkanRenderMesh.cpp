...
VulkanRenderMesh::VulkanRenderMesh(VulkanRenderDevice* vulkanRenderDevice, const GAL::RenderMesh::RenderMeshCreateInfo& renderMeshCreateInfo)
{
	size_t vertex_buffer_size = renderMeshCreateInfo.VertexCount * renderMeshCreateInfo.VertexLayout->GetSize();
	size_t index_buffer_size = renderMeshCreateInfo.IndexCount * sizeof(GTSL::uint16);
	...
	GTSL::byte* mapped_staging_buffer_memory = nullptr;
	vkMapMemory(vulkanRenderDevice->GetVkDevice(), staging_memory, 0, buffer_size, 0, reinterpret_cast<void**>(&mapped_staging_buffer_memory));
...
void VulkanRenderMesh::Destroy(GAL::RenderDevice* renderDevice)
{
