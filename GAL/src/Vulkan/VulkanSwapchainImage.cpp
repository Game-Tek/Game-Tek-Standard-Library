...
#include "GAL/Vulkan/VulkanSwapchainImage.h"

#include "GAL/Vulkan/Vulkan.h"

VulkanSwapchainImage::VulkanSwapchainImage(VulkanRenderDevice* device, const RenderTargetCreateInfo& imageCreateInfo, const VkImage image) : VulkanRenderTargetBase(imageCreateInfo)
{
	VkImageViewCreateInfo vk_image_view_create_info{ VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO };
	vk_image_view_create_info.format = ImageFormatToVkFormat(imageCreateInfo.Format);
