...
VulkanTexture::VulkanTexture(VulkanRenderDevice* vulkanRenderDevice, const GAL::TextureCreateInfo& textureCreateInfo) : Texture(textureCreateInfo)
{
	...
	GTSL::FixedVector<VkFormat> formats(2, vulkanRenderDevice->GetTransientAllocationsAllocatorReference());
	formats.EmplaceBack(FormatToVkFormat(textureCreateInfo.ImageFormat)); formats.EmplaceBack(VK_FORMAT_R8G8B8A8_UNORM);
	...
	GTSL::uint64 originalTextureSize = textureCreateInfo.ImageDataSize;
...
void VulkanTexture::Destroy(GAL::RenderDevice* renderDevice)
{
